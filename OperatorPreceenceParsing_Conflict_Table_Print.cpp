...
map<pair<char,char>, set<char>>parsing_table;   // Parsing table -- indexed on (T, T) --> Relation b/w them( <, >, =) 
char start_symbol;
int is_suitable=1;
...
void table_print()
{
	...
	for(set<char>::iterator i=term.begin(); i!=term.end(); i++)
	{
		...
		for(set<char>::iterator j=term.begin(); j!=term.end(); j++)
		{
			...
			{
				for(set<char>::iterator itr=parsing_table[pr].begin(); itr!=parsing_table[pr].end(); itr++)
					cout<<*itr;
				cout<<"\t";
			}
		}
...
void parse_string()
{
	...
	for(int i=0;i<str.size();i++)
	{
		...
		if(parsing_table.find(pr)!=parsing_table.end() && (*(parsing_table[pr].begin())=='<'||*(parsing_table[pr].begin())=='=') )
		{
		...
		else if(parsing_table.find(pr)!=parsing_table.end() && *(parsing_table[pr].begin())=='>')
		{
			// cout<<" Reduce ";
			string ss;
			...
			while(parsing_table.find(pr)!=parsing_table.end() && (*(parsing_table[pr].begin())=='='||*(parsing_table[pr].begin())=='>') )
			{
